...
export function parseFurnitureBlob(key: number, blob: Uint8Array): FurnitureItem {
  ...
  const nameLen = u32LE(blob, off); off += 8
  const name = nameLen > 0 ? readAscii(blob, off, nameLen) : ''
  off += nameLen

  const unkU64 = u64LE(blob, off); off += 8

  const roomLen = u32LE(blob, off); off += 8
  const room = roomLen > 0 ? readAscii(blob, off, roomLen) : ''
  off += roomLen

  const x = dv.getInt32(off, true); off += 4
  const y = dv.getInt32(off, true); off += 4
  const z = u32LE(blob, off); off += 4
  const flag1 = u32LE(blob, off); off += 4
  const flag2 = u32LE(blob, off); off += 4
